...
class Package:
    def __init__(self, pkg_path,product_name,usr_id,usr_pwd):
        self.path = pkg_path
        self.filename = ""
        self.name = ""
    ...
    def __set_pkg_info(self):
        ...
        if rst is None:
            ...
            try:
                package_version = re.findall(r"versionCode='([\d._]+)'", rst)[0]
                self.versioncode = package_version
            except Exception as e:
                logging.warning("[pkg_info] failed to regex package version from {}. {}".format(rst, e))
